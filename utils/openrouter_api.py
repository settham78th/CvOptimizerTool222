...
def optimize_cv(cv_text, job_description):
    ...
    1. Dokładnie analizuj i prezentuj rzeczywiste doświadczenie zawodowe:
       ...
       - Podkreślaj realne umiejętności zdobyte na każdym stanowisku
       - Używaj precyzyjnego języka branżowego
       - Zachowuj spójność w opisach i formatowaniu

    2. Zachowaj i optymalizuj prawdziwe informacje:
       - KRYTYCZNE: Zachowaj dokładnie wszystkie dane kontaktowe (imię, nazwisko, telefon, email)
       - Nie dodawaj fikcyjnych danych ani osiągnięć
       - Bazuj wyłącznie na faktycznych doświadczeniach
       - Wydobywaj wartość z istniejącego doświadczenia
       - Prezentuj informacje w sposób profesjonalny i rzeczowy
       - Zachowaj chronologię i nazwy firm z oryginalnego CV
    ...
    # Wyczyść i sformatuj tekst przed wysłaniem
    cv_text = cv_text.replace('\n\n\n', '\n\n').strip()
    cv_text = ' '.join(line.strip() for line in cv_text.split('\n') if line.strip())

    response = send_api_request(prompt, max_tokens=2500)

    # Sprawdź czy odpowiedź zawiera podstawowe informacje z oryginalnego CV
    if response:
        original_keywords = set(word.lower() for word in cv_text.split() if len(word) > 3)
        response_keywords = set(word.lower() for word in response.split() if len(word) > 3)
        if len(original_keywords & response_keywords) < len(original_keywords) * 0.5:
            logger.warning("Generated CV might be too different from original")
            raise ValueError("Generated CV does not match original content sufficiently")
